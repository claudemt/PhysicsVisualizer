...
studio_style('apply_grid', left_grid, 'panel');
...
cla(ax,'reset');
studio_style('apply_axes', ax, 'Box', 'off', 'Grid', 'off');
try, ax.BackgroundColor = style.canvasBackground; catch, end
axis(ax,[0 1 0 1]);
axis(ax,'off');
text(ax,0.5,0.58,char(string(title_text)), ...
    ...
    'FontWeight','bold', ...
    'FontSize',max(style.fontSize, round(0.72 * style.axesFontSize)), ...
    'Interpreter','none');
