function result = compute_chladni_circular(params)
if ~isfield(params, 'xi0') || isempty(params.xi0), params.xi0 = 0; end
if ~isfield(params, 'boundary') || isempty(params.boundary), params.boundary = 'free'; end
...
if isAnnulus
    ...
    bcTag = parse_annulus_boundary(params.boundary);
else
    bcTag = parse_disk_boundary(params.boundary);
    xi0 = 0;
end
...
modes = collect_circular_modes(params.nu, params.k, bcTag, xi0);
kUse = min(params.k, numel(modes));
items = cell(1, kUse);
...
for i = 1:kUse
    ...
    if isAnnulus
        titleText = local_annulus_title(params.nu, xi0, bcTag, m, s, lam_i);
        prefix = 'annulus';
    else
        titleText = local_circ_title(params.nu, bcTag, m, s, lam_i);
        prefix = 'circ';
    end

    item = render_result('heatmap', x, y, U, ...
        'Title', titleText, ...
        'XLabel', '$x$', 'YLabel', '$y$', ...
        ...
        'ZeroContour', true);
    if isAnnulus
        item.circleRadii = [xi0 1];
        item.filename = sprintf('chladni_annulus_%s_m%d_s%d.png', lower(bcTag), m, s);
    else
        item.circleRadii = 1;
        item.filename = sprintf('chladni_disk_%s_m%d_s%d.png', lower(bcTag), m, s);
    end
    items{i} = item;
end
