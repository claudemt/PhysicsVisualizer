function presets = params_preset(name)
%PARAMS_PRESET Built-in presets for FourierOptics-matlab.
%   PRESETS = PARAMS_PRESET() returns all presets.
%   P = PARAMS_PRESET(NAME) returns the preset whose Name matches NAME.
...
presets = finalize_presets([p1, p2, p3, p4, p5, p6, p7, p8, p9]);
...
name = string(name);
idx = find(strcmpi({presets.Name}, name), 1, 'first');
if isempty(idx)
...
p = struct( ...
    ...
    'fourier_plot_half_range_mm', 0.30, ...
...
function presets = finalize_presets(presets)
first = finalize_single(presets(1));
out = repmat(first, size(presets));
out(1) = first;
for k = 2:numel(presets)
    out(k) = finalize_single(presets(k));
end
presets = out;
end
...
validateattributes(p.n_samples, {'numeric'}, {'scalar','integer','>=',256});
