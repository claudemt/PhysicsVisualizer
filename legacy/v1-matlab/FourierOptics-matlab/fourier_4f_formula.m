function result = fourier_4f_formula(params, object_fun, phase_fun, filter_fun)
%FOURIER_4F_FORMULA Core 4f Fourier optics simulation.
%   RESULT = FOURIER_4F_FORMULA(PARAMS, OBJECT_FUN, PHASE_FUN, FILTER_FUN)
%   constructs the object plane, phase plane and Fourier filter plane, then
%   computes the filtered 4f image.
...
U_after_phase = object_amp .* phase_support .* exp(1i .* phase_rad);
U_fourier = fftshift(fft2(ifftshift(U_after_phase)));
U_image = fftshift(ifft2(ifftshift(U_fourier .* filter_amp)));

object_amp = object_amp ./ max(object_amp(:) + eps);
after_phase_amp = abs(U_after_phase);
after_phase_amp = after_phase_amp ./ max(after_phase_amp(:) + eps);
intensity_image = abs(U_image).^2;
intensity_image = intensity_image ./ max(intensity_image(:) + eps);

spectrum_intensity = abs(U_fourier).^2;
spectrum_intensity = spectrum_intensity ./ max(spectrum_intensity(:) + eps);
wrapped_phase = angle(exp(1i .* phase_rad));
wrapped_phase(phase_support < 0.5) = 0;
...
result.object_amp = object_amp;
result.phase_rad = phase_rad;
result.phase_wrapped = wrapped_phase;
result.phase_support = phase_support;
result.after_phase_amp = after_phase_amp;
result.spectrum_intensity = spectrum_intensity;
result.filter_amp = filter_amp;
result.output_intensity = intensity_image;
result.field_after_phase = U_after_phase;
result.field_fourier = U_fourier;
result.field_image = U_image;
