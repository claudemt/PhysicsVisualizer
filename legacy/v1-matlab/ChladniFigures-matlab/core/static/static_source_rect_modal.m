function result = static_source_rect_modal(nu, n, normalizeForDisplay, outputFolder, boundary, a, b, loadSpec, kModes, D, drawZeroContour)
...
if nargin < 11 || isempty(drawZeroContour), drawZeroContour = true; end
if nargin < 10 || isempty(D), D = 1.0; end
if nargin < 9 || isempty(kModes), kModes = 80; end
if nargin < 8 || isempty(loadSpec), loadSpec = struct('type','points','sources',[0 0 1 0]); end
if nargin < 7 || isempty(b), b = 1.0; end
if nargin < 6 || isempty(a), a = 2.0; end
if nargin < 5 || isempty(boundary), boundary = 'SSSS'; end
...
ax = gca;
set(ax, 'Color', [1 1 1]);
[Uf, climLimits, cbMode, cbInfo] = static_field_for_display(Ustatic, normalizeForDisplay);
imagesc(ax, x, y, Uf);
set(ax, 'YDir', 'normal');
axis(ax, 'equal');
...
hold(ax, 'on');
if drawZeroContour && has_zero_level(Ustatic)
    contour(ax, x, y, Ustatic, [0 0], 'k-', 'LineWidth', 1.0);
end
apply_latex_formatting(fig, ax);
set(ax, 'Layer', 'top');
xi0 = b / a;
title(ax, sprintf('$\\mathrm{%s}\\quad \\nu=%.4g\\quad \\xi_0=%.4g\\quad \\mathrm{%s}$', ...
    plain_load_label(loadSpec), nu, xi0, upper(meta.code)), 'Interpreter','latex');

filename = fullfile(outputFolder, sprintf('static-rect-%s-nu%s-xi%s.png', ...
    upper(meta.code), local_num_tag(nu), local_num_tag(xi0)));
...
result.storage_folder = outputFolder;
