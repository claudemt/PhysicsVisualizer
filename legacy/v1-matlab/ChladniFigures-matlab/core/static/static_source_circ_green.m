function result = static_source_circ_green(nu, n, normalizeForDisplay, outputFolder, boundary, xi0, loadSpec, mMax, D, drawZeroContour, distributionSamples)
...
if nargin < 11 || isempty(distributionSamples), distributionSamples = 28; end
if nargin < 10 || isempty(drawZeroContour), drawZeroContour = true; end
if nargin < 9 || isempty(D), D = 1.0; end
if nargin < 8 || isempty(mMax), mMax = 50; end
if nargin < 7 || isempty(loadSpec), loadSpec = struct('type','points','sources',[0.35 0 1 0]); end
if nargin < 6 || isempty(xi0), xi0 = 0; end
if nargin < 5 || isempty(boundary), boundary = 'C'; end
if nargin < 4 || isempty(outputFolder), outputFolder = fullfile(pwd, '.cache'); end
...
ax = gca;
set(ax, 'Color', [1 1 1]);
[Uf, climLimits, cbMode, cbInfo] = static_field_for_display(U, normalizeForDisplay);
UfImg = Uf; UfImg(~mask) = 0;
hImg = imagesc(ax, x, y, UfImg);
set(ax, 'YDir', 'normal');
axis(ax, 'equal');
axis(ax, [-R R -R R]);
set(hImg, 'AlphaData', double(mask), 'AlphaDataMapping', 'none');
color_bar(ax, 'Location', 'eastoutside', 'Interpreter', 'latex', 'Limits', climLimits);
hold(ax, 'on');
if drawZeroContour && has_zero_level(U(mask))
    contour(ax, x, y, U, [0 0], 'k-', 'LineWidth', 1.0);
end
apply_latex_formatting(fig, ax);
if isAnnulus
    title(ax, sprintf('$\\mathrm{%s}\\quad \\nu=%.4g\\quad \\xi_0=%.4g\\quad \\mathrm{%s}$', ...
        plain_load_label(loadSpec), nu, xi0, upper(bcTag)), 'Interpreter', 'latex');
    filename = fullfile(outputFolder, sprintf('static-annulus-%s-nu%s-xi%s.png', ...
        upper(bcTag), local_num_tag(nu), local_num_tag(xi0)));
else
    title(ax, sprintf('$\\mathrm{%s}\\quad \\nu=%.4g\\quad \\mathrm{%s}$', ...
        plain_load_label(loadSpec), nu, upper(bcTag)), 'Interpreter', 'latex');
    filename = fullfile(outputFolder, sprintf('static-circ-%s-nu%s-xi0.png', ...
        upper(bcTag), local_num_tag(nu)));
end
...
result.file = filename; result.files = {filename}; result.storage_folder = outputFolder;
...
function plot_display_sources(~, ~, ~)
% Intentionally empty: static figures are heat maps only.
end
