...
function local_render_compare_plot(ax, result, idx)
    ...
    switch idx
        case 1
            ...
            local_finish_2d_axes(ax);
            local_add_case_legend(ax, colors, labels, false, 'northeast');
        ...
        case 2
            if strcmp(result.baseMode, 'free')
                ...
                local_add_case_legend(ax, colors, labels, true, 'northeast');
            end
...
function outDir = local_export_outputs(result, inputData, outputRoot, mainFunctionName)
    ...
    if local_is_multi_result(result)
        outDir = fullfile(outputRoot, sprintf('%s_multi_%s', result.baseMode, stamp));
        names = { ...
    ...
    for k = 1:7
        ...
        drawnow;
        [~, fileBaseName, ~] = fileparts(names{k});
        fileBase = fullfile(outDir, fileBaseName);
        print(f, fileBase, '-dpng', '-r240');
        close(f);
    end
...
function local_style_axes(ax)
    ...
    if isprop(ax, 'TitleFontSizeMultiplier')
        ax.TitleFontSizeMultiplier = 1.35;
    end
    if isprop(ax, 'LabelFontSizeMultiplier')
        ax.LabelFontSizeMultiplier = 1.10;
    end
...
function local_apply_legend(ax, location)
    ...
    local_style_legend(lgd);
    local_place_legend_near_corner(ax, lgd);
end
